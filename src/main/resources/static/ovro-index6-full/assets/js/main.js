...
function renderSkillCards() {
    ...
    const skillCatalog = [
        ...
        {
            category: 'Frameworks and API Stack',
            items: [
        ...
        {
            category: 'Practices and Domains',
            items: [
...
function initPortfolioLightbox() {
    ...
    const openLightbox = (sourceImage) => {
        ...
        lightboxImage.alt = sourceImage.alt || 'Project preview image';
        resetView();
...
function initContactModal() {
    ...
    const validateForm = () => {
        ...
        if (!payload.fromEmail) {
            ...
            errors.fromEmail = 'Provide a valid email address.';
        }
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (!apiUrl) {
            setStatus('The contact API URL is missing from the page configuration.', 'error');
            return;
        }
